...
class MidiPlayer():
    ...
    def __init__(self, filename: str):
        ...
        self.procon = hid.Device(0x057E, 0x2009)
    ...
    def send_cmd(self, cmd: int, subcmd: list):
        self.procon.write(b''.join([b'\x01', bytes([self.counter]), b'\x00\x01\x40\x40\x00\x01\x40\x40', bytes([cmd]), bytes(subcmd)]))
    ...
    def send_midi(self):
        for msg in self.mid:
            ...
            if not msg.is_meta and msg.type == 'note_on':
                if msg.channel % 2 == 0:
                    self.procon.write(b''.join([b'\x10', bytes([self.counter]), bytes(self.miditorumble(msg.note, msg.velocity)), bytes(self.miditorumble(msg.note, msg.velocity))]))
                #else:
